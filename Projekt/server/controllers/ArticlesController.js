    ...
    addArticle: function(req, res, next) {
        ...
        redisClient.get("next_article_id", function(err, next) {
            if (err) {
                ...
                res.send({
                    "code": 500,
                    "msg": "Fehler beim auslesen des Artikelzählers!",
                    "type": "err"
                });
            }
            ...
            redisClient.rpush(ARTICLES, newArticle.id, function(err, reply) {
                if (err) {
                    ...
                    res.send({
                        "code": 500,
                        "msg": "Fehler beim hinzufügen des Artikels!",
                        "type": "err"
                    });
                    console.log(err);
                }
                ...
                if (category) {
                    redisClient.rpush(category, newArticle.id, function(err, reply) {
                        if (err) {
                            res.status(500).setHeader('Content-Type', 'application/json');
                            res.send({
                                "code": 500,
    ...
    delArticle: function(req, res, next) {
        ...
        redisClient.lrange(ARTICLES, 0, -1, function(err, reply) {
            ...
            if (_articleID === null) {
                ...
                res.send({
                    "code": 500,
                    "msg": "Zu löschender Artikel existiert nicht.",
                    "type": "err"
                });
            } else {
                var _articleById = 'article:' + _articleID;
                redisClient.get(_articleById, function(err, article) {
                    if (err) {
                        res.status(500).setHeader('Content-Type', 'application/json');
                        res.send({
                            "code": 500,
                    ...
                    redisClient.lrem(article.category, 0, _articleID, function(err) {
                        if (err) {
                            res.status(500).setHeader('Content-Type', 'application/json');
                            res.send({
                                "code": 500,
